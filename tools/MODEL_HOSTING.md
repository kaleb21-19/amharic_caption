...
A new model gets a new id (first 12 hex of `model.bin`'s SHA-256, printed by the
build as `manifest <id>`). Upload the new files (a new GitHub release tag), then
rebuild. Customers download the new model once; the old folder can be deleted.
