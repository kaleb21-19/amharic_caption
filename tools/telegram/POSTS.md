# Amharic Captions — Launch & Announcement Posts (Telegram)

Copy-paste ready. Post these when you launch, and reuse later for reach.

---

## Post A — Product announcement (use as first launch post)

> 🎬 **የአማርኛ ካፕሽን ሶፍትዌር ተጀመረ!**
>
> Premiere Pro ላይ ቪዲዮህን በራስ-ሰር **በአማርኛ ካፕሽን** አስቀምጥ!
>
> ✅ የአማርኛ ንዑስ ርዕስ በቦታው ይፈጠር
> ✅ በኮምፒውተርህ ላይ ብቻ ይሰራል — ኢንተርኔት አያስፈልግም, ቅጽበት (offline, on-device)
> ✅ Windows & Mac
> ✅ አንድ ጊዜ ክፍያ: **ETB 2,500** (subscription የለም)
>
> 🎁 **ሁሉም ሰው 2 ነጻ trial** ያገኛል — ሞክር እና እርካታ ከተሰማህ ግዛ!
>
> 🏦 ለመግዛት: ባንክ ዝውውር → **KALEB TEGEGEN** (CBE 1000504159977 / Abyssinia 402393939 / Zemen 1031111343277015) → Machine ID + payment screenshot ላክ
> 👇 የተሟላ መረጃ pinned message ላይ
>
> ማስታወቂያ ወደውት ባልንጀሮችህ አጋራ! 🚀

---

## Post B — "How it works" demo (attach a short video/screenshot)

> 🎥 የአማርኛ ካፕሽን ሶፍትዌር በስራ ላይ! 👇
> (paste screenshot/video of Amharic captions on a Premiere timeline)
>
> 1. ፓነሉን ክፈት
> 2. ክሊፕ ምረጥ
> 3. "Generate Captions" ተጫን
> 4. አማርኛ ካፕሽን በቦታው ✨
>
> ለ2 ነጻ trial ፓነሉን ሞክር! 💬

---

## Post C — Pricing reminder (repeat every few days)

> 💰 **ዋጋ: ETB 2,500 ብቻ** (አንድ ጊዜ ክፍያ)
>
> • Windows & Mac ላይ ይሰራል
> • ኢንተርኔት ሳያስፈልግ, በኮምፒውተርህ ላይ ብቻ
> • የአማርኛ ካፕሽን በአውቶማቲክ
>
> 🎁 2 ነጻ trial አለ — መግዛትህ በፊት ሞክር!
> 🏦 ባንክ ዝውውር → **KALEB TEGEGEN** (CBE 1000504159977 / Abyssinia 402393939 / Zemen 1031111343277015) → screenshot + Machine ID ላክ

---

## Post D — Social proof / testimonial (after someone buys)

> 🎉 ተጠቃሚዎቻችን የአማርኛ ካፕሽን ሶፍትዌርን በአማርኛ ቪዲዮ ስራ እየተጠቀሙ ነው!
>
> "በቀድሞው ሰዓት ካፕሽን ለመስራት ብዙ ጊዜ ይወስድ ነበር፤ አሁን በደቂቃዎች ውስጥ ይጨርሳል።" — satisfied user
>
> አንተስ ሞክር እና ተመስገን! 💬

---

## Install steps post (make this sticky for buyers)

> 🛠 **እንዴት እንደሚጫኑ** (Install steps)
>
> ⚠️ Premiere Pro **2024 (v24) or newer** ያስፈልጋል (older versions are NOT supported).
>
> 1. ለኮምፒውተርህ የሚሆነውን ፋይል ከ
>    https://github.com/kaleb21-19/amharic-captions-downloads/releases/latest አውርድ
>    (Windows → `amharic-captions-win-x64.zip`)
> 2. ይዘቱን አውጣ (extract)። ፎልደሩን በ:
>    `%AppData%\Adobe\CEP\extensions\` ስር አስቀምጥ
>    (ስሙ `com.amharic.captions` መሆን አለበት)
>    💡 ወይም፦ ፎልደሩ ውስጥ ያለውን `Install.cmd` ላይ ዳብል-ክሊክ አድርግ —
>    ሁሉንም በራሱ ያደርጋል (admin አያስፈልግም)!
> 3. Premiere Pro ከፍት → ፕሮጀክት ክፈት (project ሳይከፍት Extensions ሜኑ ግራይ ይሆናል) → Windows > Extensions > "Amharic Captions" ክፈት
> 4. License ለማግኘት Machine ID ቅድህ።
>
> ከተጫነ በኋላ 2 ነጻ trial አለ! 🎁
> ችግር ካጋጠመ → DM 👍
