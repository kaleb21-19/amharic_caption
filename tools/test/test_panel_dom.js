...
await t('1. load: theme, runtime, version, font pill, health rows, onboarding', async () => {
  ...
  try {
    ...
    assert.strictEqual(p.els('panelVersion').textContent, '1.4.22');
...
await t('8. review: low-confidence cues get the highlight, confident ones don\'t', async () => {
  ...
  try {
    ...
    try {
      ...
      assert.strictEqual(p.els('reviewFlagCount').textContent, '1', 'flag badge counts only the flagged cue');
      assert.strictEqual(p.els('reviewNextFlag').hidden, false, 'nav button shown when something is flagged');
    } finally { p.close(); }
  } finally { restoreCache(snap); }
});

await t('9. review: jump-to-next-flagged cycles through low-confidence cues and wraps', async () => {
  const fixture = path.join(REPO, 'tools', 'test', 'fixtures', 'twospeaker.wav');
  const key = cacheKeyFor(fixture, { cap:'words', group:3, chars:42, speakers:false });
  const snap = snapshotCache();
  try {
    const base = snap !== null ? JSON.parse(snap) : {};
    const srt3 = ['1','00:00:01,000 --> 00:00:02,000','አንድ','',
                  '2','00:00:02,000 --> 00:00:03,000','ሁለት','',
                  '3','00:00:03,000 --> 00:00:04,000','ሶስት',''].join('\n');
    // Cues 0 and 2 are flagged; cue 1 (confident) sits between them.
    base[key] = { srt: srt3, transcript: '', confs: [0.05, 0.9, 0.02], at: Date.now() };
    restoreCache(JSON.stringify(base));
    ...
    try {
      ...
      assert.strictEqual(p.els('reviewFlagCount').textContent, '2', 'two flagged cues counted');
      const jumpToNextFlag = p.evalVm('jumpToNextFlag');
      const list = p.els('reviewList');

      jumpToNextFlag();
      assert.strictEqual(p.evalVm('reviewFlagCursor'), 0, 'first jump lands on the first flagged cue');
      jumpToNextFlag();
      assert.strictEqual(p.evalVm('reviewFlagCursor'), 2, 'second jump advances to the next flagged cue');
      jumpToNextFlag();
      assert.strictEqual(p.evalVm('reviewFlagCursor'), 0, 'third jump wraps back to the first');

      // Filtering to just "ሁለት" (the confident middle cue) leaves no flagged
      // cue visible — jumping must be a no-op, not throw or reset the cursor.
      p.els('reviewSearch').value = 'ሁለት';
